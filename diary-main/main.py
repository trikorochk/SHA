...
class User(db.Model):
    ...
    passwod=db.Column(db.String(16), nullable=False)
...
def login():
        ...
        if request.method == 'POST':
            ...
            form_passwod = request.form['password']
            
            #Задание №4. Реализовать проверку пользователей
            users_db = User.query.all()
            for user in users_db:
                if form_login == user.login and form_passwod == user.passwod:
                    return redirect('/index')
                else: 
                    error='ОШИБКА повитря на тревога'
...
def reg():
    if request.method == 'POST':
        ...
        user = User(login=login, passwod=passwod)
        db.session.add(user)
        db.session.commit()
